...
router.get('/', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    res.render('admin/dashboard', {
      ...
      users: usersList,
      seo: {
        title: 'لوحة تحكم الإدارة - HM CAR',
        description: 'لوحة تحكم إدارة نظام HM CAR لبيع وشراء السيارات',
        breadcrumbs: {
          items: [
            { name: 'الرئيسية', url: '/cars' },
...
router.post('/brands', requireAdmin, brandUpload.single('logo'), async (req, res) => {
  try {
    ...
    await Brand.create({ name, logoUrl });
    req.flash('success', 'تمت إضافة الوكالة بنجاح.');
    res.redirect('/admin/brands');
  } catch (error) {
    console.error(error);
    req.flash('error', 'حدث خطأ أثناء إضافة الوكالة.');
    res.redirect('/admin/brands');
  }
});
...
router.get('/spare-parts', requireAdmin, async (req, res) => {
  const [parts, spareBrands] = await Promise.all([
    SparePart.find().populate('spareBrand').sort({ createdAt: -1 }),
    SpareBrand.find().sort({ name: 1 })
  ]);
...
router.post('/spare-brands', requireAdmin, (req, res, next) => {
  brandUpload.single('logo')(req, res, (err) => {
    if (err) {
      const msg = err.message === 'Invalid file type'
        ? 'نوع الملف غير مسموح. ارفع صور بصيغة JPG/PNG/WEBP فقط.'
        : 'تعذر رفع الشعار. تأكد أن حجم الصورة لا يتجاوز 3MB.';
      if (req.session) req.session.flash = { type: 'danger', message: msg };
      return res.redirect('/admin/spare-brands');
    }
    next();
  });
}, async (req, res) => {
  const name = String(req.body.name || '').trim();
  const logoUrlRaw = String(req.body.logoUrl || '').trim();
  ...
  if (!name) {
    if (req.session) req.session.flash = { type: 'danger', message: 'يرجى إدخال اسم الشركة.' };
    return res.redirect('/admin/spare-brands');
  }

  const key = name.toLowerCase();
  await SpareBrand.findOneAndUpdate(
    { key },
    { name, key, logoUrl },
    { upsert: true, new: true }
  );

  if (req.session) req.session.flash = { type: 'success', message: 'تم حفظ الشركة بنجاح.' };
  return res.redirect('/admin/spare-brands');
});

router.post('/spare-brands/:id/delete', requireAdmin, async (req, res) => {
  await SpareBrand.findByIdAndDelete(req.params.id);
  await SparePart.updateMany({ spareBrand: req.params.id }, { $set: { spareBrand: null } });
  if (req.session) req.session.flash = { type: 'success', message: 'تم حذف الشركة.' };
  return res.redirect('/admin/spare-brands');
});
