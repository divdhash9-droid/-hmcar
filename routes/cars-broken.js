...
router.get('/', async (req, res) => {
  ...
  // Apply advanced caching with multi-level strategy and refresh-ahead
  autoCacheMiddleware({
    ttl: 300, // 5 minutes for car listings
    methods: ['GET'],
    excludePaths: [], // Cache all car listing requests
    cacheAuthenticated: false // Don't cache for authenticated users to show personalized data
  }),
  async (req, res) => {
    // صفحة قائمة السيارات: فلاتر + ترقيم مع التخزين المؤقت المتقدم
    const {
      q,
      make,
      model,
    ...
    // تحسين: استخدام text search بدلاً من regex للاستفادة من text index
    if (q && String(q).trim()) {
    ...
    // Use refresh-ahead strategy for car data to serve stale data while refreshing
    const [carsResult, totalResult, categories] = await Promise.all([
      refreshAhead(`cars:list:${JSON.stringify(filter)}:${skip}:${limit}`, async () => {
      ...
      refreshAhead(`cars:count:${JSON.stringify(filter)}`, async () => {
...
router.post('/new',
  requireAuth,
  requireRole('admin'),
  // Invalidate car listings cache when new car is added
  cacheInvalidationMiddleware(['cars:list', 'cars:brands']),
  (req, res, next) => {
...
router.post('/:id/edit',
  requireAuth,
  requireRole('admin'),
  // Invalidate car listings cache when car is updated
  cacheInvalidationMiddleware(['cars:list', 'cars:brands']),
