> [[ARABIC_HEADER]] هذا الملف (STRUCTURE.md) جزء من مشروع HM CAR ويحتوي تعليقات عربية لضمان الوضوح.
...
## 🧭 ملخص المجلدات السريع

| المجلد | الغرض الرئيسي |
|--------|----------------|
| `config/` | إعدادات الاتصال بقواعد البيانات، التخزين السحابي، Redis، ومفاتيح الخدمات الحساسة |
| `middleware/` | طبقات الحماية والتخزين المؤقت والتحسينات التي تعمل قبل وصول الطلبات للمسارات |
| `models/` | تعريفات Mongoose لكافة الكيانات (سيارات، مزادات، مستخدمين، تقارير، إلخ) |
| `routes/` | جميع واجهات HTTP وواجهات لوحة التحكم مقسمة بحسب المجال الوظيفي |
| `services/` | منطق الأعمال القابل لإعادة الاستخدام مثل الدفع، الإشعارات، مراقبة الأداء، والويب سوكيت |
| `views/` | قوالب EJS الكاملة للواجهة الأمامية بما فيها لوحات الأدمن والأجزاء المشتركة |
| `public/` | ملفات ثابتة (CSS/JS/صور) بالإضافة إلى manifest و Service Worker لميزة PWA |
| `scripts/` | سكربتات الصيانة، التهيئة، الأتمتة، وفحوصات الجودة التي تُشغّل من سطر الأوامر |
| `utils/` | وظائف صغيرة داعمة مثل تحسين الصور أو تنسيق التواريخ |
| `test/` | مساحة للاختبارات الآلية (تُترك فارغة إذا لم تُستخدم) |
| `logs/` | مخرجات مراقبة النظام أثناء التطوير (يُفضّل تجاهلها في Git) |
| `uploads/` | ملفات المستخدمين المرفوعة محلياً، ويُنصح باستعمال Cloudinary/Storage في الإنتاج |
...
- `scripts/initializeSystem.js`: إعداد الأدوار الافتراضية، الصلاحيات، وإعدادات الموقع بعد التثبيت الأول.
- `scripts/addDatabaseIndexes.js`: إنشاء الفهارس الأساسية على مجموعات السيارات، المزادات، والمستخدمين لتحسين الأداء.
- `scripts/optimizeDatabaseIndexes.js`: مراجعة الفهارس الموجودة وتنظيف غير المستخدم منها قبل الإنتاج.
- `scripts/add-arabic-headers.js`: يولّد التعليقات العربية الموحدة أعلى ملفات المشروع لتسهيل التوثيق.
- `scripts/check-ejs.js`: يفحص قوالب EJS ويتأكد من عدم وجود أخطاء بناء أو متغيرات ناقصة قبل النشر.
- `scripts/clean-project.(ps1|sh)`: حذف node_modules، السجلات، وملفات الرفع لضمان بداية نظيفة.
- `scripts/auto-deploy.(js|sh)`: تنفيذ نشر أوتوماتيكي محلي مع التحقق من الحالة عبر أوامر موحدة.
- `scripts/reset-database.js` و `scripts/simple-seed.js`: إعادة تهيئة قاعدة البيانات وملئها ببيانات تجريبية متناسقة.
