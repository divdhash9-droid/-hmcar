...
export default function CarsBrowserPage() {
    return (
        <Suspense fallback={<div className="min-h-screen bg-[#050505] text-white flex items-center justify-center font-black uppercase tracking-[0.5em] italic animate-pulse">Syncing Machinery...</div>}>
            <CarsContent />
        </Suspense>
    );
}
...
function CarsContent() {
    ...
    return (
        <div className={cn("min-h-screen bg-[#050505] text-white selection:bg-luxury-gold selection:text-black", isRTL && "font-arabic")} dir={isRTL ? 'rtl' : 'ltr'}>
            <Navbar />

            {/* Cinematic Background */}
            <div className="fixed inset-0 pointer-events-none z-0">
                <div className="absolute top-0 right-0 w-full h-[800px] bg-gradient-to-b from-luxury-gold/5 via-transparent to-transparent opacity-40" />
            ...
            <main className="relative z-10 max-w-[1600px] mx-auto px-6 pt-32 pb-24">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
                    <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
                        <div className="flex items-center gap-3 text-luxury-gold/60 mb-4">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.4em]">{isRTL ? "مستودع السيارات" : "VEHICLE TERMINAL"}</span>
                        </div>
                        <h1 className="text-5xl md:text-7xl font-black uppercase italic tracking-tighter leading-none">
                            {isRTL ? "تصفح" : "BROWSE"} <span className="text-luxury-gold">{isRTL ? "المخزون" : "INVENTORY"}</span>
                        </h1>
                    </motion.div>
                    ...
                    >
                        <div className="px-4 py-2 border-r border-white/10">
                            ...
                            <span className="text-[10px] text-white/30 uppercase block font-bold">{isRTL ? "سيارة متاحة" : "ASSETS"}</span>
                        </div>
                        <button
                ...
                <AnimatePresence>
                    {showFilters && (
                        ...
                        >
                            <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-8 grid grid-cols-1 md:grid-cols-4 gap-8">
                                {/* Search */}
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-white/30 uppercase tracking-widest">{isRTL ? "بحث نصي" : "TEXT SEARCH"}</label>
                                ...
                                {/* Agency */}
                                <div className="space-y-4">
                                    <label htmlFor="agency-select" className="text-[10px] font-black text-white/30 uppercase tracking-widest">{isRTL ? "الوكالة (الماركة)" : "AGENCY (MAKE)"}</label>
                                    <select
                                        id="agency-select"
                                        title={isRTL ? "اختر الوكالة" : "Select Agency"}
                                ...
                                {/* Price Range */}
                                <div className="space-y-4">
                                    <label className="text-[10px] font-black text-white/30 uppercase tracking-widest">{isRTL ? "النطاق السعري" : "PRICE SPECTRUM"}</label>
                                ...
                                <div className="flex items-end">
                                    ...
                                    >
                                        <X className="w-4 h-4" />
                                        {isRTL ? "تصفير الفلاتر" : "CLEAR SETTINGS"}
                                    </button>
                                </div>
                            </div>
                        </motion.div>
                ...
                {loading && page === 1 ? (
                    ...
                    <div className="py-48 text-center bg-white/[0.01] border border-dashed border-white/10 rounded-[3rem]">
                        <div className="w-20 h-20 bg-white/5 rounded-full flex items-center justify-center mx-auto mb-8 border border-white/10">
                            <Car className="w-10 h-10 text-white/10" />
                        </div>
                        <h2 className="text-3xl font-black uppercase italic tracking-tighter mb-4">{isRTL ? "لا توجد نتائج" : "OFF-LINE"}</h2>
                        <p className="text-white/40 text-[10px] font-bold uppercase tracking-[0.3em]">{isRTL ? "جرب تعديل خيارات البحث" : "RECONFIGURE SEARCH PARAMETERS"}</p>
                    </div>
                ) : (
                    <>
                                    ...
                                    <Link href={`/cars/${car.id || car._id}`} className="block obsidian-card obsidian-card-hover overflow-hidden rounded-[2.5rem]">
                                        ...
                                        <div className="relative h-72 w-full bg-zinc-900">
                                            <Image
                                                ...
                                                alt={car.title} fill className="object-cover grayscale transition-all duration-[1s] group-hover:grayscale-0 group-hover:scale-110 opacity-60 group-hover:opacity-100"
                                            />
                                        ...
                                        <div className="p-8 space-y-6">
                                            <div className="space-y-2">
                                                <span className="text-[8px] font-black text-luxury-gold/50 tracking-[0.4em] uppercase">{typeof car.make === 'object' ? car.make?.name : car.make}</span>
                                            ...
                                            <div className="flex items-center justify-between pt-6 border-t border-white/5 mt-auto">
                                                <div>
                                                    <span className="text-[8px] font-black text-white/20 tracking-widest uppercase block mb-1">MARKET VALUE</span>
                                                    <div className="text-2xl font-black italic gold-glow leading-none">
                                                        {Number(car.price).toLocaleString()} <span className="text-[10px] not-italic text-white/40 ml-1">SAR</span>
                                                    </div>
                                                </div>
