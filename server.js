...
// Site settings middleware (cached)
// Ensures footer social links + WhatsApp number are available on all pages.
let cachedSiteFooter = null;
let cachedSiteFooterAt = 0;
...
app.use(async (req, res, next) => {
  try {
    const now = Date.now();
    if (!cachedSiteFooter || (now - cachedSiteFooterAt) > SITE_SETTINGS_CACHE_MS) {
      const settings = await Settings.getSettings();
      const footerRaw = (settings && settings.footer) ? settings.footer : {};
      const footer = JSON.parse(JSON.stringify(footerRaw || {}));

      // WhatsApp number: prefer footer.whatsappNumber, fallback to whatsapp.phoneNumber
      const waCandidate = footer.whatsappNumber || (settings && settings.whatsapp && settings.whatsapp.phoneNumber) || '';
      footer.whatsappNumber = normalizeWhatsAppNumber(waCandidate);

      cachedSiteFooter = footer;
      cachedSiteFooterAt = now;
    }

    res.locals.siteSettings = cachedSiteFooter || {};
  } catch (error) {
    // Never block rendering if settings cannot be loaded
...
app.get('/', async (req, res) => {
  try {
    ...
    if (global.localDB) {
      ...
      // جلب إعدادات الموقع (محملة مسبقاً عبر middleware)
      siteSettings = (res.locals && res.locals.siteSettings) ? res.locals.siteSettings : {};
...
async function ensureDevAdminUser() {
  ...
  const mongoUriRaw = String(process.env.MONGO_URI || process.env.MONGODB_URI || '').trim();
  const mongoUri = mongoUriRaw.toLowerCase();

  // Skip if explicitly offline mode
  if (mongoUri.startsWith('offline://') || mongoUri === 'localdb' || mongoUri === 'local-db') return;

  const usesMemoryMongo = mongoUri.startsWith('memory://');
  const enabled = parseBool(process.env.ENABLE_DEV_ADMIN, true);
  if (!enabled) return;

  const isLocalMongo = usesMemoryMongo ||
    mongoUri.includes('mongodb://127.0.0.1') ||
    mongoUri.includes('mongodb://localhost') ||
    mongoUri.includes('mongodb://mongodb:');

  // If the dev server is pointed at a remote/shared MongoDB, require explicit opt-in.
  if (!isLocalMongo && !parseBool(process.env.FORCE_DEV_ADMIN, false)) return;

  const email = String(process.env.DEV_ADMIN_EMAIL || 'admin@local.test').trim().toLowerCase();
  const password = String(process.env.DEV_ADMIN_PASSWORD || 'admin123456');
  const name = String(process.env.DEV_ADMIN_NAME || 'Local Admin').trim() || 'Local Admin';
  ...
  console.log('\n=== Dev Admin Login ===');
  console.log(`URL: http://localhost:${serverConfig.port}/auth/login?admin=true`);
...
async function connectToDatabase() {
  ...
  try {
    ...
    // By default, do NOT start in offline mode; this causes non-persistent data.
    // If you really want offline mode, set MONGO_URI=offline:// (or enable this flag).
    const allowOffline = parseBool(process.env.ALLOW_OFFLINE_MODE, false);
    if (process.env.NODE_ENV !== 'production' && allowOffline) {
      console.warn('⚠️ Starting app in offline mode (ALLOW_OFFLINE_MODE=1) - database features will be limited');
      return null;
    }
    throw error;
  }
