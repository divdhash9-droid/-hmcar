...
router.get('/brands', async (req, res) => {
  const brands = await Brand.find({ forSpareParts: true }).sort({ name: 1 });
...
router.get('/meta', async (req, res) => {
  ...
  try {
    ...
    if (!brandKey && !make) {
      const brands = await Brand.find({ forSpareParts: true }).sort({ name: 1 }).select('name');
    ...
    const brand = await Brand.findOne({
      $and: [
        { forSpareParts: true },
        { $or: [{ key: effectiveKey }, { name: new RegExp('^' + effectiveKey.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '$', 'i') }] }
      ]
    });
    ...
    if (!model) {
      const models = await SparePart.distinct('carModel', {
        ...(brand ? { brand: brand._id } : (make ? { carMake: makeRe } : {})),
        carModel: { $nin: [null, ''] }
      });
...
router.get('/', async (req, res) => {
  ...
  if (brandKey) {
    ...
    const b = await Brand.findOne({ key, forSpareParts: true });
    if (b) {
      filter.brand = b._id;
